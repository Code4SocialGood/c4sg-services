...
import org.c4sg.service.FeedService;
...
public class AllForGoodFeedController {
	...
	public FeedDTO getProjects() {
	    ...
	    afg.setFeedInfo(feedService.addFeedInfo());
		List<ProjectDTO> projects = new ArrayList<ProjectDTO>(); 
		for (OrganizationDTO o : organizations) {
			o.setLocation(feedService.populateFeedLocation(o));
			for (int i = 0; i < o.getProjects().size(); i++) {
				o.getProjects().get(i).setDateTimeDurations(feedService.populateDateTimeDurations());
				List<FeedSponsoringOrganizationDTO> sponsoringOrganizations = new ArrayList<FeedSponsoringOrganizationDTO>();
				sponsoringOrganizations.add(feedService.populateSponsoringOrganization(o, i));
				o.getProjects().get(i).setSponsoringIds(sponsoringOrganizations);
				feedService.populateRemoteFlag(o, i);
